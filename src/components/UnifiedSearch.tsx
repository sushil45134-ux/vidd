    ...
    <div className="pt-6 px-4 md:px-12 min-h-screen bg-[#141414]">
      <div className="max-w-3xl mx-auto mb-8">
        <div className="flex items-center bg-[#222] border border-gray-700 rounded-lg overflow-hidden focus-within:border-[#e50914] transition-colors">
          ...
          />
          {query && (
            <button onClick={() => setQuery("")} className="text-gray-500 hover:text-white px-3">
              <X size={18} />
            </button>
          )}
        </div>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filtered.map((m) => (
            <div
              ...
              tabIndex={0}
              role="button"
              aria-label={`${m.title} — open details`}
              onKeyDown={(e) => {
            ...
            >
              <div className="legacy-media relative bg-gray-900">
                ...
                />
                <div
                  className={`absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors items-center justify-center ${IS_TV ? "hidden" : "flex"}`}
                >
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
              ...
              <div className="p-3">
                ...
                <p className="text-gray-500 text-xs truncate">{m.genre.slice(0, 2).join(" • ")}</p>
              </div>
            </div>
          ))}
        </div>
